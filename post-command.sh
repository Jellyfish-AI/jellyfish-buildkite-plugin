...
# The PLUGIN_SLUG is derived from the plugin's name in plugin.yml,
...
REFERENCE_ID="${BUILDKITE_PLUGIN_JELLYFISH_BUILDKITE_PLUGIN_REFERENCE_ID:-$BUILDKITE_BUILD_ID}"
DEPLOYMENT_NAME="${BUILDKITE_PLUGIN_JELLYFISH_BUILDKITE_PLUGIN_NAME:-$BUILDKITE_PIPELINE_SLUG}"
REPO_NAME="${BUILDKITE_PLUGIN_JELLYFISH_BUILDKITE_PLUGIN_REPO_NAME:-$BUILDKITE_REPO}"
SOURCE_URL="${BUILDKITE_PLUGIN_JELLYFISH_BUILDKITE_PLUGIN_SOURCE_URL:-$BUILDKITE_BUILD_URL}"
...
HTTP_RESPONSE=$(curl -s -w "%{http_code}" -X POST "$WEBHOOK_URL" \
  ...
  -H "X-jellyfish-api-token: $API_TOKEN" \
  -d "$JSON_PAYLOAD")
